<template>
    <view :class="['tab', { 'tab-fixed': fixed }]">
        <scroll-view :class="['tab-bar', { 'tab-bar-center': tabCenter }]" :scroll-into-view="scrollId" scroll-with-animation
                     scroll-x="true">
            <view v-for="(item, index) in tabList" :id="`tab_${index}`" :key="index"
                  :class="{ 'active': tabIndex == index }"
                  :style="[{ 'padding': `0 ${tabSpacing}rpx` }, tabIndex == index ? activeTextStyle : defaultTextStyle]"
                  class="tab-bar-item" @click="putChange(item.name, index); tabChange(index)">
                <text class="tab-label">{{ item.label }}</text>
            </view>
        </scroll-view>
        <view :style="{ 'transform': `translateX(${translateX}%)`, 'transition': `transform ${d / 1000}s ease-in-out` }"
              class="tab-cont">
            <slot></slot>
        </view>
    </view>
